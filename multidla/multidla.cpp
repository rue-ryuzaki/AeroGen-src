...
void printField(multidla::XField* fld)
{
...
MCoordVec* createNeighborsMap(size_t dimensions)
{
...
MCoord freeRandomPntInField(multidla::XField* fld)
{
    MCoord sz = fld->size();
    MCoord res;
    while (true) {
...
double vAdd(multidla::XField* fld, const MCoord& currCoord)
{
    double r = 0.5385;
    double h = r - 0.5;
    double result = 0.0;
...
FieldElement deepestMark(std::map<FieldElement, FieldElement>& substitute,
                         FieldElement mark)
{
...
multidla::XField* markClusters(const multidla::XField* fld)
{
    ...
    auto* result = new multidla::XField(fld->size(), fld->nullPnt(), fld->radius() * 2);
...
std::vector<MCoordVec>* extractClusters(multidla::XField* MarkedFld)
{
...
void changeLabels(multidla::XField* fld, double fraction, FieldElement lbl)
{
...
void setClusterVal(MCoordVec* cluster, multidla::XField* fld, FieldElement value)
{
...
void removeCluster(MCoordVec* cluster, multidla::XField* fld)
{
    setClusterVal(cluster, fld, FREE_CELL);
}

void restoreCluster(MCoordVec* cluster, multidla::XField* fld)
{
    setClusterVal(cluster, fld, OCUPIED_CELL);
}

bool isClusterInField(MCoordVec* cluster, multidla::XField* fld)
{
    for (auto pnt = cluster->begin(); pnt != cluster->end(); ++pnt) {
...
bool isAggregation(MCoordVec* cluster, multidla::XField* fld, MCoordVec* directions)
{
    for (auto pnt = cluster->begin(); pnt != cluster->end(); ++pnt) {
...
double MultiDLA::surfaceArea(double density, uint32_t steps) const
{
    ...
    if (m_finished) {
        ...
        auto* cf = new multidla::XField(sc, MCoord(), 2 * m_fld->radius());
...
void MultiDLA::clusterAggregation(multidla::XField* fld, size_t clusterCnt)
{
    ...
    uint32_t maxSize = 0;
    {
        auto* marked = markClusters(fld);
        auto* clusters = extractClusters(marked);
    ...
    while (true) {
        ...
        auto* cluster = new MCoordVec(clusters->at(randomCluster).begin(),
                                      clusters->at(randomCluster).end());
        removeCluster(cluster, fld);
        delete clusters;
        for (uint32_t i = 0; i < maxClusterMoves; ++i) {
            //cout << "Move " << i << endl;
            auto* newCluster = moveCluster(cluster, directions);
